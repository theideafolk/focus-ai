/*
  ...
  2. Changes
    - Make `start_date` NOT NULL to ensure it's required
...
DO $$
BEGIN
  -- Add project_type field with check constraint
  IF NOT EXISTS (
  ...
  -- Add project_type_other field for custom types
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
  ...
  -- Add user_priority field (1-5 scale)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
  ...
  IF NOT EXISTS (
    ...
    -- Add check constraint for valid complexity values
